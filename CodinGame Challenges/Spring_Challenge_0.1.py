...
#pellet = [x,y,value]
def distance(pellet, pacx, pacy):
    dx = pellet[0] - pacx
    dy = pellet[1] - pacy
    d = (dx**2 + dy**2)**.5
    return d

#pellets is (list of list of the pellet information)
def closestP(pellets, superpellets, pacx, pacy):
    x = 0; y = 0;
    min = 999999
    print(len(superpellets), file=sys.stderr, flush=True)
    if(len(superpellets) == 0):
        for p in pellets:
            d = distance(p, pacx, pacy)
            if(d < min):
                min = d
                x = p[0]
                y = p[1]
...
# game loop
while True:
    pellets = [] #list of lists [[x,y,value], ...]
    superpellets = [] #same thing but supers
    ...
    # MOVE <pacId> <x> <y>
    print("MOVE 0 ", end="")
    print(*closestP(pellets, superpellets, x, y))
